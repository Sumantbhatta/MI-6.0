...
private ProjectService projectService;
   ...
   public ResponseEntity<MachinaryMgmtBaseApiResponse> updateEquipment(Long id, @Valid EquipmentRequestDto equipmentRequestDto)
      ...
      Project project = projectService.findById(equipmentRequestDto.getProjectId())
                .orElseThrow(() -> new Exception("Project not found"));
      EquipmentCategory equipmentcategory= equipmentCategoryService.findById(equipmentRequestDto.getCategoryId())
      .orElseThrow(() -> new Exception("Equipment Category not found"));
      Model model= modelService.findById(equipmentRequestDto.getModelId())
      .orElseThrow(() -> new Exception("Model Category not found"));
      equipmentMapper.updateEntityFromDto(equipmentRequestDto, existingEquipment);
     existingEquipment.setProject(project);
     existingEquipment.setModel(model);
     existingEquipment.setCategory(equipmentcategory);
